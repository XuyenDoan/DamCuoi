 ...
 */
const props = defineProps<{ milestone: LoveStoryMilestone }>()
...
async function removePhoto(photoId: string) {
  const idx = props.milestone.photos.findIndex((p) => p.id === photoId)
  if (idx === -1) return
  ...
  <Teleport to="body">
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" @click.self="emit('close')">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Chỉnh sửa mốc thời gian"
        class="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-xl border border-secondary-light/40 bg-bg p-6 shadow-lg"
      >
