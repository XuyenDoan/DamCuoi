...
// Lời chúc mới gửi mặc định `approved: false` — phải admin bấm "Duyệt" mới
// hiển thị công khai (phản hồi thật: sợ khách gửi lời bậy bạ). Dữ liệu CŨ
// (gửi trước khi có kiểm duyệt) không có field này, coi như đã duyệt sẵn.
const pendingWishes = computed(() => (wishes.value ?? []).filter((w) => w.approved === false))
const reviewedWishes = computed(() => (wishes.value ?? []).filter((w) => w.approved !== false))

async function approveWish(wish: Wish) {
  pendingId.value = wish.id
  try {
    await $fetch(`/api/admin/wishes/${wish.id}/approve`, { method: 'POST' })
    await refresh()
  } finally {
...
async function deleteWish(wish: Wish) {
  const confirmMessage =
    wish.approved === false
      ? `Từ chối và xoá lời chúc của "${wish.name}"? Hành động không thể hoàn tác.`
      : `Xoá lời chúc của "${wish.name}"? Hành động không thể hoàn tác.`
  ...
  <div>
    ...
    <div class="mx-auto max-w-3xl px-6 py-10">
      ...
      <p class="mb-6 text-sm text-text-muted">
        Duyệt lời chúc mới gửi trước khi hiển thị công khai, ẩn hoặc xoá lời chúc không phù hợp.
      </p>
      ...
      <template v-else>
        <section v-if="pendingWishes.length > 0" class="mb-10">
          <h3 class="mb-1 font-heading text-lg text-text">Chờ Duyệt</h3>
          <p class="mb-4 text-sm text-text-muted">
            Khách mời đã gửi {{ pendingWishes.length }} lời chúc — duyệt để hiển thị công khai.
          </p>

          <ul class="flex flex-col gap-4">
            <li v-for="wish in pendingWishes" :key="wish.id" class="rounded-xl border border-primary/30 bg-primary/5 p-5">
              ...
              <div class="mt-4 flex gap-2">
                <button
                  type="button"
                  class="focus-ring rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white transition-colors duration-200 hover:bg-primary/90 disabled:pointer-events-none disabled:opacity-50"
        ...
        <section>
          <h3 v-if="pendingWishes.length > 0" class="mb-4 font-heading text-lg text-text">Đã Duyệt</h3>
          <div v-if="reviewedWishes.length === 0" class="py-10 text-center text-text-muted">
            Chưa có lời chúc nào được duyệt.
          </div>
          <ul v-else class="flex flex-col gap-4">
            <li
              v-for="wish in reviewedWishes"
              :key="wish.id"
