...
- **Component `LotusScene`** — khung cảnh ao sen phủ **TOÀN TRANG**, vẽ lại bằng SVG vector (không dùng ảnh có sẵn vì bản quyền + không điều khiển được animation từng cánh):
  ...
  - **Mỗi bông có đúng 1 lá sen đi kèm ngay dưới/cạnh nó** (yêu cầu chỉnh lần 5: "chỗ nào có hoa thì thêm lá phía dưới") — lá gắn trực tiếp vào từng object bông (`leaf: {left, top/bottom, size, tone}`) thay vì rải độc lập như bản trước, tổng 8 lá khớp 1-1 với 8 bông. Lá luôn render TRƯỚC bông trong DOM (nằm dưới lớp bông khi xếp chồng).
