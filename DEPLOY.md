...
9. **`kex_exchange_identification: Connection closed by remote host`** ở bước đẩy code lên server (21/07) — server đóng kết nối SSH **ngay lúc bắt tay, trước cả bước xác thực khoá** (không phải lỗi sai khoá/secret). Điều tra: tải log job thất bại qua GitHub API bằng token có sẵn trong git credential manager (`git credential fill`) vì `gh` CLI không có sẵn trong môi trường — log xác nhận đúng lỗi trên ngay ở lệnh `ssh` đầu tiên. Kiểm chứng thêm bằng SSH thủ công từ máy khác (IP quen thuộc với server) tới đúng host/user: bắt tay + tới bước xác thực bình thường (chỉ báo sai khoá vì không có key đúng) — chứng tỏ `sshd` trên server vẫn hoạt động tốt, vấn đề chỉ xảy ra với kết nối từ GitHub Actions. Nguyên nhân nhiều khả năng nhất: cPHulk (chống brute-force mặc định của cPanel) trên hosting chia sẻ chặn IP lạ khi thấy nhiều kết nối SSH liên tiếp trong thời gian ngắn — bản trước mở **3 kết nối SSH riêng biệt** (rm/mkdir, tar pipe, restart) từ 1 IP hoàn toàn mới mỗi lần chạy (IP của GitHub Actions runner đổi mỗi lần), trông giống hành vi dò quét. Khắc phục: dùng **SSH connection multiplexing** (`ControlMaster=auto` + `ControlPersist=60s` + `ControlPath`) — chỉ bắt tay/xác thực đúng 1 lần, các lệnh sau tái dùng cùng 1 kết nối qua socket cục bộ; kèm retry 5 lần cho lần kết nối đầu (nếu cPHulk chỉ chặn tạm theo cửa sổ thời gian). **Nếu vẫn lỗi sau khi đã có multiplexing + retry**: đây là dấu hiệu server đang chặn hẳn (không phải chặn tạm) — cần vào cPanel → Security → "cPHulk Brute Force Protection" kiểm tra danh sách IP bị chặn/nới lỏng độ nhạy, vì AI code không có quyền truy cập cPanel để tự sửa phần này.
